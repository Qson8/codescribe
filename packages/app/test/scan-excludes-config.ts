...
import { DEFAULT_EXCLUDES, DEFAULT_EXTENSIONS, discover, normalizeExcludeRules } from '@codescribe/core';
import {
...
const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codescribe-scan-excludes-'));
...
const scanRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'codescribe-scan-excludes-project-'));
