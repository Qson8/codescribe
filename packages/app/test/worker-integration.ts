...
import { defaultCleanOptions, type CleanedFile, type FileCandidate, type FileEntry } from '@codescribe/core';
...
async function main() {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'codescribe-worker-integration-'));
