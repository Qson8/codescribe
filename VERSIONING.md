...
本文是 CodeScribe 产品版本、配置兼容、规则追踪和 Git 发布的唯一标准。README 只保留摘要；发生冲突时以本文和自动校验脚本为准。
...
| 配置结构版本 | `1` | 迁移 `.codescribe.json` | `CONFIG_SCHEMA_VERSION` |
...
- patch 版本必须保持 `.codescribe.json` 向后兼容
...
7. 创建 annotated tag：`git tag -a v<version> -m "CodeScribe v<version>"`
